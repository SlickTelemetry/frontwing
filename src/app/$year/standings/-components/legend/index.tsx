...
import { useHiddenItems } from './context';

export const Legend = () => {
  const params = useParams({ strict: false });
  const year = params.year != null ? String(params.year) : '0';
  const is2026 = 2026 <= parseInt(year, 10);
...
export const DriverFilters = () => {
  const search = useSearch({ strict: false }) as { chart?: string };
  const showDrivers = search.chart !== 'constructors';
